...
function PokerGame({ balance, setBalance }) {
  ...
  const determineWinner = (pot) => {
    ...
    if (winner.length > 1) {
      ...
      setBalance((prevMoney) => prevMoney + pot / 2); // Return the player's bet
    } else if (winner[0] === dealerHand) {
      resultMessage += 'Dealer wins!';
    } else {
      resultMessage += 'Player wins!';
      setBalance((prevMoney) => prevMoney + pot); // Add the entire pot to the player's money
    }
